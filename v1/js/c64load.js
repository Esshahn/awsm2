/*    
    ...
    for use with the codef framework
      ...
      INIT
      c64 = new C64Load();
      c64.init(filename,canvas,x,y,flicker,func);
      
      filename = filename to display
      canvas = target canvas
      x = x position on target canvas
      y = y position on target canvas
      flicker = enable (1) or disable (0) a flickering effect
...
function C64Load(){
  ...
  this.init = function(game,targetCanvas,xPos,yPos,flicker,callFunctionWhenDone){
      ...
      this.flicker_gfx = new image("gfx/flicker.png");
  ...
  this.flickerScreen = function(){
    ...
    this.flicker_gfx.draw(this.targetCanvas,0,ypos);
  }
  ...
  this.run = function(){
    ...
    if(typeof(this.currentAnimStage) == "undefined") this.currentAnimStage = 1;
      ...
      this.targetCanvas.fill(this.colors64["light_blue"]);
      this.canvas.quad(0,0,320,200,this.colors64["blue"]);
    ...
    if (this.flicker == 1) this.flickerScreen();
  ...
  this.stage5 = function(){ 
    ...
    var keys = Object.keys(this.colors64);
 
    for(var i = 0; i <= this.targetCanvas.height; i+=4){      
      this.targetCanvas.quad(0,i,this.targetCanvas.width,4,this.colors64[keys[Math.floor(keys.length * Math.random())]] );      
    }
